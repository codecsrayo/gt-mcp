...
Minimal Rust CLI to invoke **gt-core MCP** tools over Streamable HTTP. Handles the
SSE handshake (`initialize` → capture `Mcp-Session-Id` → `notifications/initialized`)
that Claude Code's deferred-tool loader does not, so all 57 tools + 2 resources are
